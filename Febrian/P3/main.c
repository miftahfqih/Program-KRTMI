...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
void jalan(int motor, int speed);
int kinematic(int arah, int x, int y, int z);
...
void jalan(int motor, int speed){
	...
	if(motor == 0){
		HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, dir_a);
		HAL_GPIO_WritePin(GPIOA, GPIO_PIN_12, dir_b);
		TIM14 -> CCR1 = speed;
	}
	else if(motor == 1){
		HAL_GPIO_WritePin(GPIOC, GPIO_PIN_9, dir_a);
		HAL_GPIO_WritePin(GPIOC, GPIO_PIN_8, dir_b);
		TIM10 -> CCR1 = speed;
	}
	else if(motor == 2){
		HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, dir_a);
		HAL_GPIO_WritePin(GPIOB, GPIO_PIN_2, dir_b);
		TIM14 -> CCR2 = speed;
	}
	else if(motor == 3){
		HAL_GPIO_WritePin(GPIOC, GPIO_PIN_4, dir_a);
		HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13, dir_b);
		TIM14 -> CCR1 = speed;
	}
}
...
void move(int x, int y, int z){
	int m0 = kinematic(0, x, y, z);
	int m1 = kinematic(1, x, y, z);
	int m2 = kinematic(2, x, y, z);
	int m3 = kinematic(3, x, y, z);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
	...
	else if(integral <= -999) integral = -999;

	output = proportional + integral + derivative;
	if(output >= 999) output = 999;
	else if(output <= -999) output = -999;
